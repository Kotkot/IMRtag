...
#aaaa <- view(subset(Data_mackerel_all, subset=c(is.na(dist)==TRUE & !is.na(recapturedate)==TRUE)))
...
## Need to make some geogaphical division
...
#ggmap(map_area) + geom_point(data=Data_mackerel_final, aes(x=lo, y=la)) +
#  geom_point(data=Data_mackerel_final, aes(x=cLon, y=cLat), col="red")
...
### There is something going on....
  ...
  ## Tagging location and recapture
  # Map + centre of gravity
...
### Model this with respect to the different covariates
  ...
  #### Limiting the analysis to releases around Ireland:
      ...
      ## Using duration (not standardized) but focusing on within year recapture
        ...
        ## Do some explanatory analysis: There is something going on...
        ...
        ## This makes me think that I might need to develop a mixture model (with K components)
    ...
    # With 3 groups
        ...
        options(mc.cores = 3)
        rstan_options(auto_write = TRUE)
        mixture_mod3 <- stan( file = 'C:/Users/a23092/Documents/Projects/Pelagic/IMRtag/src/mackerel_mvt_model_threshold_2breaks.stan', data = Data3,
                             iter = 5000 , warmup=3500 , chains = 3, thin=10,
                             control = list(adapt_delta = 0.99, max_treedepth = 15), seed=123)
        ...
        library(shinystan)
        launch_shinystan(mixture_mod3)
        check_hmc_diagnostics(mixture_mod3)     # need no warning
        get_low_bfmi_chains(mixture_mod3)
        stan_diag(mixture_mod3)
        stan_rhat(mixture_mod3)

        sampler_params <- get_sampler_params(mixture_mod3, inc_warmup = FALSE)
        ...
        #Data_mackerel_use_Ireland_select <- subset(Data_mackerel_final_sameyear,  Tag_area %in% c("West_Ireland", "North_Ireland"))
...
#### Need to perform some tag recapture modeling
...
############ Some plotting of mark recapture pattern
...
# ##
    ...
    Select_release_bio <- Mark_recap_bio
    West_Ireland <- unique(Data_mackerel_all$ices)[unlist(sapply(31:36, function(x) grep(x, unique(Data_mackerel_all$ices))))]
    North_Ireland <- unique(Data_mackerel_all$ices)[unlist(sapply(37:46, function(x) grep(x, unique(Data_mackerel_all$ices))))]
    Bergen <- unique(Data_mackerel_all$ices)[grep("F", unique(Data_mackerel_all$ices))]
    Iceland <- unique(Data_mackerel_all$ices)[grep(58, unique(Data_mackerel_all$ices))]
    Select_release_bio <- Select_release_bio %>% mutate(Tag_area = ifelse(ices_rectangle %in% West_Ireland, "West_Ireland",
    ...
    Select_release_bio <- Select_release_bio %>% mutate(Tag_area_large = ifelse(ices_rectangle %in% c(West_Ireland,North_Ireland), "Ireland",
                                                                              ifelse(ices_rectangle %in% Bergen, "Bergen",
                                                                                     ifelse(ices_rectangle %in% Iceland, "Iceland", "Weird"))))

    Select_release_bio$Tag_area <- as.factor(Select_release_bio$Tag_area)
    Select_release_bio$Tag_area_large <- as.factor(Select_release_bio$Tag_area_large)

    Select_release_bio <- subset(Select_release_bio, Tag_area_large == "Ireland")
    Select_release_bio$Release_year <- as.factor(lubridate::year(as.Date(Select_release_bio$catch_date)))
    Select_release_bio$Release_month <- as.factor(lubridate::month(as.Date(Select_release_bio$catch_date)))
    Select_release_bio$Release_month <- factor(Select_release_bio$Release_month, labels=c("01", "05","06"))
    Select_release_bio$Release_day <- as.factor(lubridate::day(as.Date(Select_release_bio$catch_date)))

    table(Select_release_bio$Release_year)


    Catch_bio$Year <- as.factor(lubridate::year(as.Date(Catch_bio$catch_date)))

    par(mfrow=c(3,2), mar=c(2,2,1,1), oma=c(2,2,1,1))
    for (i in 2014:2019)
    {
      with(subset(Select_release_bio, Release_year == i), plot(age, length, ylim=c(0, 0.5)))
    }
