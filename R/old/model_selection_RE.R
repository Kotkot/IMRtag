...
# m1 <- lm(log_rate ~ Latitude_scaled + Latitude2_scaled + length_scaled + julian_recapture_scaled, data=Data_mackerel_use_Ireland_select)
...
# -- looping over potential models:to remove parameter 1 by one based on the largest p-value --
for(k in 1:(nrow(Maps)-1)){
  if (k==1){
    ...
    #-- which parameter estimate has the lowest p-value? downward model selection --
      ...
      if (j[1] <= ncol(XX)) {Maps[(k+1), c(j,j+ncol(XX))] <- Maps[k,j] } #; Maps2[(k+1), (j+1):ncol(Maps2)] <- Map2$beta[(j+1):ncol(Maps2)]-1 }
      if (j[1] >  ncol(XX)) {Maps[(k+1), c(j-ncol(XX),j)] <- Maps[k,j-ncol(XX)]   } #; Maps2[(k+1), (j+2):ncol(Maps2)] <- Map2$beta[(j+2):ncol(Maps2)]-1 }
...
for (k in 1:4) {
  Map_best = list()
  Map_best$beta <- factor(Maps_best[k,])
  ...
  parameters_tmb_best <- list(beta = matrix(c(rep(10,N_threshold),runif((ncol(XX)-1)*N_threshold,-2,2)),byrow=T, ncol=N_threshold),
                         ...
                         year = matrix(rep(0, length(unique(Data_mackerel_use_Ireland_select$Release_year))*N_threshold), ncol=2),
                         log_sigma_year = rep(log(0.2),N_threshold))
  parameters_tmb_best$beta[which(is.na(Maps_best[k,]))] <- 0
  ...
  Maps2[[k]] <- Map_best$beta
  (AIC_selection2[k] <- opt1break_best$objective * 2 + p * length(opt1break_best$par))
...
parameters_tmb_best <- list(beta = matrix(c(rep(10,N_threshold),runif((ncol(XX)-1)*N_threshold,-2,2)),byrow=T, ncol=N_threshold),
                            ...
                            year = matrix(rep(0, length(unique(Data_mackerel_use_Ireland_select$Release_year))*N_threshold), ncol=2),
                            log_sigma_year = rep(log(0.2),N_threshold))
parameters_tmb_best$beta[which(is.na(Maps2[[1]]))] <- 0
...
res <- data.frame(MLE=mle[1:ncol(XX),], p_value=round(pp,2))
colnames(res) <- c("mu_before", "mu_after", "p_value")
res
