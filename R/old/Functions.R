...
#' @param control A list of control parameters. For details see \code{\link[stats]{optim}}
...
#'
#' @return the standard output from \code{\link[stats]{optim}}, except with additional diagnostics and timing info,
...
function( obj,
  ...
  # Run first time
  start_time = Sys.time()
  parameter_estimates = optim( par=startpar, fn=fn, gr=gr, control=nlminb.control, lower=lower, upper=upper )
