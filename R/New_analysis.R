...
run_directionality <- function(month, best="overall", data_origin="0year", parallel=FALSE){
  ...
  m11 <- gam(list(direction ~  -1 + s(Latitude_scaled) + Release_timing_fact + Catch_year + s(Length_scaled) + s(julian_recapture_std_scaled),
                 ~  -1 + s(Latitude_scaled) + Catch_year + Release_timing_fact + s(Length_scaled) + s(julian_recapture_std_scaled)),
            family=multinom(K=2), data = www)
  m12 <- gam(list(direction ~  -1 + s(Latitude_scaled, by=Release_timing_fact) + Catch_year + s(Length_scaled) + s(julian_recapture_std_scaled),
                 ~  -1 + s(Latitude_scaled, by=Release_timing_fact) + Catch_year + s(Length_scaled) + s(julian_recapture_std_scaled)),
            family=multinom(K=2), data = www)
  ...
  #                ~  -1 + s(Latitude, by=Catch_year) + Catch_year + s(Length, by=Catch_year) + s(julian_recapture_std_scaled)),
  #           family=multinom(K=2), data = www)
  (AICs <- AIC(m0,m1,m11,m12,m2,m3,m4))
  (BICs <- BIC(m0,m1,m11,m12,m2,m3,m4))

  # plot.gam(m1, pch=19, cex=0.5, scale=0,pages = 1)
  # library(generalhoslem)
  # pred <- predict(m0, type="response")
  ...
  pred <- predict(m0, type="response", newdata=data.frame())
  ...
  out$AICs <- AICs
  out$BICs <- BICs
  out$Res_cross_validation <- Res_cross_validation
  out$Ranking <- Ranking
...
Oct_0lag <- run_directionality(month=10, data_origin="0year")
Oct_1lag <- run_directionality(month=10, data_origin="1year")
Oct_2lag <- run_directionality(month=10, data_origin="2year")
...
Feb_0lag <- run_directionality(month=14, data_origin="0year")
Feb_1lag <- run_directionality(month=14, data_origin="1year")
Feb_2lag <- run_directionality(month=14, data_origin="2year")



#### Plotting function #####

Plot_marginal <- function(month, data_origin){


  out <- readRDS(file=paste0("month=", month, "_lag=", data_origin, ".rds"))
  best_model <- out$best_model[[1]]
...
for (direction in 1:3){
  ##### marginal effect in real scale
    # Effect of latitude release
      datpred <- expand.grid(Latitude_scaled = seq(min(www$Latitude_scaled),max(www$Latitude_scaled),by=0.01), Catch_year=levels(www$Catch_year)[1], Length_scaled=0, julian_recapture_std_scaled=0)
      pred_marginal <- predict(best_model, newdata=datpred, type="response", se=TRUE)
      datpred1 <- data.frame(lat = datpred$Latitude_scaled*sd(www$Latitude)+mean(www$Latitude), y = pred_marginal$fit[,direction], se = pred_marginal$se.fit[,direction])
      datpred1 <- datpred1 %>% mutate(low = y - 1.96*se, upr=y+1.96*se)
      p1 <- ggplot(datpred1, aes(x=lat, y=y)) + geom_ribbon(aes(ymin=low, ymax=upr), fill=grey(0.8)) +
        geom_line(size=2) + theme_bw() + gg_control + labs(x="Latitude (º)", y="Marginal probability")
    # Effect of size
      datpred <- expand.grid(Latitude_scaled = 0, Catch_year=levels(www$Catch_year)[1], Length_scaled=seq(min(www$Length_scaled),max(www$Length_scaled),by=0.01), julian_recapture_std_scaled=0)
      pred_marginal <- predict(best_model, newdata=datpred, type="response", se=TRUE)
      datpred2 <- data.frame(length = datpred$Length_scaled*sd(www$Length)+mean(www$Length), y = pred_marginal$fit[,direction], se = pred_marginal$se.fit[,direction])
      datpred2 <- datpred2 %>% mutate(low = y - 1.96*se, upr=y+1.96*se)
      p2 <- ggplot(datpred2, aes(x=length, y=y)) + geom_ribbon(aes(ymin=low, ymax=upr), fill=grey(0.8)) +
        geom_line(size=2) + theme_bw() + gg_control + labs(x="Fish body size (cm)", y="Marginal probability")
    # Effect of recapture location
      datpred <- expand.grid(Latitude_scaled = 0, Catch_year=levels(www$Catch_year)[1], Length_scaled=0, julian_recapture_std_scaled=seq(min(www$julian_recapture_std_scaled),max(www$julian_recapture_std_scaled),by=0.01))
      pred_marginal <- predict(best_model, newdata=datpred, type="response", se=TRUE)
      datpred3 <- data.frame(recap = datpred$julian_recapture_std_scaled*sd(www$julian_recapture_std)+mean(www$julian_recapture_std), y = pred_marginal$fit[,direction], se = pred_marginal$se.fit[,direction])
      datpred3 <- datpred3 %>% mutate(low = y - 1.96*se, upr=y+1.96*se)
      p3 <- ggplot(datpred3, aes(x=recap, y=y)) + geom_ribbon(aes(ymin=low, ymax=upr), fill=grey(0.8)) +
        geom_line(size=2) + theme_bw() + gg_control + labs(x="Recapture date (julian days from 1/1)", y="Marginal probability")
    # Effect of catch year
      datpred <- expand.grid(Latitude_scaled = 0, Catch_year=levels(www$Catch_year), Length_scaled=0, julian_recapture_std_scaled=0)
      pred_marginal <- predict(best_model, newdata=datpred, type="response", se=TRUE)
      datpred4 <- data.frame(year = datpred$Catch_year, y = pred_marginal$fit[,direction], se = pred_marginal$se.fit[,direction])
      datpred4 <- datpred4 %>% mutate(low = y - 1.96*se, upr=y+1.96*se)
      p4 <- ggplot(datpred4, aes(x=year, y=y)) + geom_point() + geom_errorbar(aes(ymin=low, ymax=upr)) +
        theme_bw() + gg_control + labs(x="Year effect", y="Marginal probability")

  pp <- grid.arrange(p1,p2,p3,p4,nrow=2)

  datpred1$month <- month
  datpred2$month <- month
  datpred3$month <- month
  datpred4$month <- month
  datpred1$data_origin <- data_origin
  datpred2$data_origin <- data_origin
  datpred3$data_origin <- data_origin
  datpred4$data_origin <- data_origin
  datpred1$direction <- direction
  datpred2$direction <- direction
  datpred3$direction <- direction
  datpred4$direction <- direction

  Data_predall$datpred1 <- rbind(Data_predall$datpred1, datpred1)
  Data_predall$datpred2 <- rbind(Data_predall$datpred2, datpred2)
  Data_predall$datpred3 <- rbind(Data_predall$datpred3, datpred3)
  Data_predall$datpred4 <- rbind(Data_predall$datpred4, datpred4)
...
dec0 <- Plot_marginal(month=12, data_origin="0year")
dec1 <- Plot_marginal(month=12, data_origin="1year")
dec2 <- Plot_marginal(month=12, data_origin="2year")

  Dec_all <- list()
  Dec_all$datpred1 <- rbind(dec0$datpred1, dec1$datpred1, dec2$datpred1)
  Dec_all$datpred2 <- rbind(dec0$datpred2, dec1$datpred2, dec2$datpred2)
  Dec_all$datpred3 <- rbind(dec0$datpred3, dec1$datpred3, dec2$datpred3)
  Dec_all$datpred4 <- rbind(dec0$datpred4, dec1$datpred4, dec2$datpred4)

  OGR_labeller <- function(string) ifelse(string==1, "Go to South", ifelse(string == 2, "Go to Iceland", "Go to Norway"))
  p1 <- ggplot(Dec_all$datpred1, aes(x=lat, y=y, col=data_origin)) + geom_ribbon(aes(ymin=low, ymax=upr, fill=data_origin), alpha=0.2) +
    geom_line(size=2) + facet_wrap(~direction, labeller=as_labeller(OGR_labeller)) + theme_bw() + gg_control + labs(x="Latitude (º)", y="Marginal probability") + scale_fill_viridis_d() + scale_color_viridis_d()
  p2 <- ggplot(Dec_all$datpred2, aes(x=length, y=y, col=data_origin)) + geom_ribbon(aes(ymin=low, ymax=upr, fill=data_origin), alpha=0.2) +
    geom_line(size=2) + facet_wrap(~direction, labeller=as_labeller(OGR_labeller)) + theme_bw() + gg_control + labs(x="Fish body size (cm)", y="Marginal probability") + scale_fill_viridis_d() + scale_color_viridis_d()
  p3 <- ggplot(Dec_all$datpred3, aes(x=recap, y=y, col=data_origin)) + geom_ribbon(aes(ymin=low, ymax=upr, fill=data_origin), alpha=0.2) +
    geom_line(size=2) + facet_wrap(~direction, labeller=as_labeller(OGR_labeller)) + theme_bw() + gg_control + labs(x="Recapture date (julian days from 1/1)", y="Marginal probability") + scale_fill_viridis_d() + scale_color_viridis_d()
  p4 <- ggplot(Dec_all$datpred4, aes(x=year, y=y, col=data_origin)) + geom_point(position=position_dodge(width=0.5)) + geom_errorbar(aes(ymin=low, ymax=upr, col=data_origin), position=position_dodge(width=0.5), alpha=0.2) +
    theme_bw() + facet_wrap(~direction, labeller=as_labeller(OGR_labeller))+ gg_control + labs(x="Year effect", y="Marginal probability") + scale_color_viridis_d()
  ...
  Catch_year$Direction <- c(rep("Go to Iceland", nrow(Catch_year)/2), rep("Go to Norway", nrow(Catch_year)/2))
