...
########## Preparing the final data from Ireland for the analysis
  ...
  ## Doing the analysis of P(moving to Iceland) or P(moving to Norway)
  	...
  	## Analysis of the directional movement (multinomial)
        ...
        pred <- predict(m1, type="response", newdata = data.frame(Latitude= seq(51, 59, by=0.05), Length = 35, julian_recapture_std_scaled = 0, Catch_year=2014))
        pred

        plot(seq(51, 59, by=0.05), pred[,1])
        plot(seq(51, 59, by=0.05), pred[,2])
        plot(seq(51, 59, by=0.05), 1-pred[,3])


        if (best == "AIC") best_model <- list(m0,m1,m2,m3,m4)[[which.min(AICs$AIC)]]
        if (best == "Manual") best_model <- m4
        plot.gam(m2, residuals=TRUE, pch=19, cex=0.5, scale=0,pages = 2)
        ...
        ggplot(Dat_all, aes(x=x,y=y)) + geom_line(size=2) +
          ...
          facet_wrap(Region~Variable, scales="free") +
           theme_bw() + gg_control + labs(x="Standardized value", y="Estimated effect")
        ...
        aa <- table(apply(predict(mall, type="response"), 1, which.max), www$direction+1); diag(aa) <- 0; sum(aa)
        ...
        set.seed(777)
        www1 <- www %>% group_by(Catch_year) %>% fold(k = 10) %>% ungroup()
        ...
        for (it in 1:30){
          ...
          for (k in 1:10){
            ...
            # if (is(m5) != "try-error")   obs1 <- mean(apply(pred1-obs, 1, function(x) sum(x^2)))
            # if (is(m6) != "try-error")   {set.seed(x); obs6 <- apply(apply(pred6,1,function(x) rmultinom(n=100,size=1,prob=x)),2,function(x) which(x==1));nobs6 = sum(testing$direction != (obs6-1))/length(testing)}
            # if (is(m7) != "try-error")   {set.seed(x); obs7 <- apply(apply(pred7,1,function(x) rmultinom(n=100,size=1,prob=x)),2,function(x) which(x==1));nobs7 = sum(testing$direction != (obs7-1))/length(testing)}
            # if (is(m8) != "try-error")   {set.seed(x); obs8 <- apply(apply(pred8,1,function(x) rmultinom(n=100,size=1,prob=x)),2,function(x) which(x==1));nobs8 = sum(testing$direction != (obs8-1))/length(testing)}
            # if (is(mall) != "try-error") {set.seed(x); obsall <- apply(apply(predall,1,function(x) rmultinom(n=100,size=1,prob=x)),2,function(x) which(x==1));nobsall = sum(testing$direction != (obsall-1))/length(testing)}
        ...
        if (model_type == "Iceland"){
  			...
  			if (model_type == "Norway"){
  			  m1 <- gam(to_norway ~  -1 + s(Latitude) + Catch_year + s(Length, by=Catch_year) + s(julian_recapture_std_scaled, k= 3), family=binomial,
  			          data = www)
    			m2 <- gam(to_norway ~  -1 + s(Latitude, by=Release_timing_fact) + Catch_year + s(Length, by=Catch_year) + s(julian_recapture_std_scaled, k= 3), family=binomial,
    			          data = www)
    			m4 <- gam(to_norway ~  -1 + s(Latitude) + Catch_year + s(Length) + s(julian_recapture_std_scaled, k= 3), family=binomial,
    			          data = www)
    			m5 <- gam(to_norway ~  -1 + s(Latitude, by=Release_timing_fact) + Catch_year + s(Length) + s(julian_recapture_std_scaled, k= 3), family=binomial,
    			          data = www)
    			m6 <- gam(to_norway ~  -1 + s(Latitude, by=Catch_year) + s(Length) + s(julian_recapture_std_scaled, k= 3), family=binomial,
    			          data = www)
  			}

  			if (model_type == "Northsea"){
  			  m1 <- gam(to_northsea ~  -1 + s(Latitude) + Catch_year + s(Length, by=Catch_year) + s(julian_recapture_std_scaled, k= 3), family=binomial,
  			          data = www)
    			m2 <- gam(to_northsea ~  -1 + s(Latitude, by=Release_timing_fact) + Catch_year + s(Length, by=Catch_year) + s(julian_recapture_std_scaled, k= 3), family=binomial,
    			          data = www)
    			m4 <- gam(to_northsea ~  -1 + s(Latitude) + Catch_year + s(Length) + s(julian_recapture_std_scaled, k= 3), family=binomial,
    			          data = www)
    			m5 <- gam(to_northsea ~  -1 + s(Latitude, by=Release_timing_fact) + Catch_year + s(Length) + s(julian_recapture_std_scaled, k= 3), family=binomial,
    			          data = www)
    			m6 <- gam(to_northsea ~  -1 + s(Latitude, by=Catch_year) + s(Length) + s(julian_recapture_std_scaled, k= 3), family=binomial,
    			          data = www)
  			}
